...
├── patterns.py                  # Архитектурные паттерны
└── ea_0001.py                   # EA.0001 — выход в интернет (deployment)
```
...
### 8. **ea_0001.py** — EA.0001 (выход в интернет)
**Назначение**: По диаграмме развёртывания определяет, есть ли у приложения признаки доступа из интернета.

**Проверка:**
- `EA.0001` — в узлах deployment с заполненным `environment` анализируются `properties.ip` (внешние IPv4/IPv6), `properties.external_ip`, `properties.host` (суффикс `.beeline.ru`).

**Ключевые функции:**
- `check_ea_0001()` — точка входа
- `check_external_services()` — обход дерева deployment и формирование `FitnessStatus`

**Где вызывается:** только из `POST /api/v1/fitness-function/local/{docId}` (не из `/fdm` и не из `dsl2fdm`).

### 9. **container.py** - Проверка контейнерной модели
