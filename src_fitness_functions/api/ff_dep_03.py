...
router = APIRouter(tags=["/api/v1/ff/dep03"])


class Dep03Request(BaseModel):
    """Тело запроса к эндпоинту DEP.03."""
...
class Dep03Response(BaseModel):
    """Тело ответа эндпоинта DEP.03 в формате, ожидаемом FF Manager."""
    callId: UUID = Field(..., description="Должен совпадать с callId из запроса")
    isCheck: bool = Field(..., description="Прошла ли проверка")
    details: List[Dep03Detail] = Field(..., description="Детализация проверки")


@router.post("/api/v1/ff/dep03")
async def dep03_check(
    body: Dep03Request,
    docId: Optional[int] = Query(None, description="Идентификатор документа (опционально)"),
):
    """
    DEP.03: узлы развёртывания сверяются с CMDB через Products API (``cmdb_deployment_compare``).
    """
    logger.info(
        "DEP.03 — callId=%s productCode=%s docId=%s",
        body.callId,
        body.productCode,
        docId,
    )
    ...
    _d1, _d2, dep03_ok, _r1, _r2, rows_dep03 = analyze_dep_workspace(data, body.productCode)
    return Dep03Response(
        callId=body.callId,
        isCheck=dep03_ok,
        details=[Dep03Detail(**row) for row in rows_dep03],
    )
