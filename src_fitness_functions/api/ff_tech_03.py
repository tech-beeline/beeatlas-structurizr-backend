...
from src_fitness_functions.sdk.technology_utils import analyze_tech03_workspace

logger = logging.getLogger(__name__)

router = APIRouter(tags=["/api/v1/ff/tech03"])


class Tech03Request(BaseModel):
    """Тело запроса к эндпоинту TECH.03."""
...
class Tech03Response(BaseModel):
    """Тело ответа эндпоинта TECH.03 в формате, ожидаемом FF Manager."""
    callId: UUID = Field(..., description="Должен совпадать с callId из запроса")
    isCheck: bool = Field(..., description="Прошла ли проверка")
    details: List[Tech03Detail] = Field(..., description="Детализация проверки")


@router.post("/api/v1/ff/tech03")
async def tech03_check(
    body: Tech03Request,
    docId: Optional[int] = Query(None, description="Идентификатор документа (опционально)"),
):
    """
    TECH.03: у контейнеров (не landscape) задано поле technology.
    """
    logger.info(
        "TECH.03 — callId=%s productCode=%s docId=%s",
        body.callId,
        body.productCode,
        docId,
    )
    ...
    tech03_ok, rows = analyze_tech03_workspace(data, body.productCode)
    return Tech03Response(
        callId=body.callId,
        isCheck=tech03_ok,
        details=[Tech03Detail(**row) for row in rows],
    )
