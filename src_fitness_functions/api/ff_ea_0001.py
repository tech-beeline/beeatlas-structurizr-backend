...
from src_fitness_functions.sdk.ea_utils import analyze_ea_workspace

logger = logging.getLogger(__name__)

router = APIRouter(tags=["/api/v1/ff/ea0001"])


class Ea0001Request(BaseModel):
    """Тело запроса к эндпоинту EA.0001."""
...
class Ea0001Response(BaseModel):
    """Тело ответа эндпоинта EA.0001 в формате, ожидаемом FF Manager."""
    callId: UUID = Field(..., description="Должен совпадать с callId из запроса")
    isCheck: bool = Field(..., description="Прошла ли проверка")
    details: List[Ea0001Detail] = Field(..., description="Детализация проверки")


@router.post("/api/v1/ff/ea0001")
async def ea0001_check(
    body: Ea0001Request,
    docId: Optional[int] = Query(None, description="Идентификатор документа (опционально)"),
):
    """
    EA.0001: выход приложения в интернет по узлам развёртывания (см. ``check_external_services``).
    """
    logger.info(
        "EA.0001 — callId=%s productCode=%s docId=%s",
        body.callId,
        body.productCode,
        docId,
    )
    ...
    ea_ok, rows_ea = analyze_ea_workspace(data, body.productCode)
    return Ea0001Response(
        callId=body.callId,
        isCheck=ea_ok,
        details=[Ea0001Detail(**row) for row in rows_ea],
    )
